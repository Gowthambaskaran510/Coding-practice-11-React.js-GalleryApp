...
class Gallery extends Component {
  ...
  getFilterImage = () => {
    ...
    const filterImage = imagesList.filter(
      eachItem => eachItem.thumbnailUrl === activeImage,
    )
  ...
  render() {
    const {activeImage} = this.state
    const {imageUrl, imageAltText} = imagesList[activeImage]

    return (
      <div className="bg-container">
        <div className="card-container">
          <img src={imageUrl} alt={imageAltText} className="l-img" />
          ...
          <ul className="thumbnail-container">
            {imagesList.map(eachItem => (
              <ThumbnailItem
                thumbnailImg={eachItem}
                key={eachItem.id}
                updatedImg={this.updatedImg}
                isActive={activeImage === eachItem.id}
              />
            ))}
          </ul>
        </div>
